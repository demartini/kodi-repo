...
class Generator:
    ...
    def _generate_addons_file(self):
        ...
        for addon in addons:
            try:
                if not os.path.isdir(addon) or addon == "zips" or addon == "docs" or addon.startswith("."):
                    continue
        ...
        # self._save_file(addons_xml.encode("utf-8"), file="addons.xml", decode=True)

    def _generate_md5_file(self):
        try:
            m = hashlib.md5(
                open(os.path.join("zips", "addons.xml"), "r", encoding="utf-8").read().encode("utf-8")
